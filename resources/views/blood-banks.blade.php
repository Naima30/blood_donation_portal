...
<div class="container my-5">
    ...
    <div class="mb-4 text-center">
        <h3 class="fw-bold text-danger">🩸 Blood Banks in Kochi</h3>
        <p class="text-muted">
            Search verified hospitals, explore locations on the map, and get directions instantly.
        </p>
    </div>
    ...
    <div class="search-wrapper mb-4">
        ...
        <ul id="suggestions" class="list-group shadow-sm"></ul>
    </div>
    ...
    <div class="alert alert-info mt-5">
        <strong>Note:</strong> Locations open directly in Google Maps using verified hospital names.
    </div>
...
<!-- STYLES -->
<style>
.search-wrapper { position:relative; z-index:2000; }
...
hospitals.forEach(h=>{
 let marker=L.marker([h.lat,h.lng]).addTo(map).bindPopup(`<strong>${h.name}</strong>`);
 markers[h.id]=marker;
});
...
searchInput.addEventListener('keyup',function(){
 ...
 let matches=hospitals.filter(h=>h.name.toLowerCase().startsWith(value));
